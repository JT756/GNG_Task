...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\Josef Haydn\\Desktop\\SemesterVI\\AP\\APG3_lastrun.py',
...
# Initialize components for Routine "Type_Group"
Type_GroupClock = core.Clock()
...
# ------Prepare to start Routine "Type_Group"-------
...
Type_GroupComponents = [Input_Group, Fed_Text]
for thisComponent in Type_GroupComponents:
...
Type_GroupClock.reset(-_timeToFirstFrame)  # t0 is time of first possible flip
frameN = -1

# -------Run Routine "Type_Group"-------
while continueRoutine:
    # get current time
    t = Type_GroupClock.getTime()
    tThisFlip = win.getFutureFlipTime(clock=Type_GroupClock)
...
# the Routine "Type_Group" was not non-slip safe, so reset the non-slip timer
routineTimer.reset()

# set up handler to look after randomisation of conditions etc
L1 = data.TrialHandler(nReps=2, method='sequential', 
...
for thisL1 in L1:
    ...
    for thisComponent in Trial_InstructionsComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    L1.addData('TR_Instructions.started', TR_Instructions.tStartRefresh)
    L1.addData('TR_Instructions.stopped', TR_Instructions.tStopRefresh)
    # check responses
    if key_resp.keys in ['', [], None]:  # No response was made
        key_resp.keys = None
    L1.addData('key_resp.keys',key_resp.keys)
    if key_resp.keys != None:  # we had a response
        L1.addData('key_resp.rt', key_resp.rt)
    L1.addData('key_resp.started', key_resp.tStartRefresh)
    L1.addData('key_resp.stopped', key_resp.tStopRefresh)
    # the Routine "Trial_Instructions" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    
    # set up handler to look after randomisation of conditions etc
    L2 = data.TrialHandler(nReps=1, method='sequential', 
        extraInfo=expInfo, originPath=-1,
    ...
    for thisL2 in L2:
        ...
        if TR_Key.keys in ['', [], None]:  # No response was made
            TR_Key.keys = None
        L2.addData('TR_Key.keys',TR_Key.keys)
        if TR_Key.keys != None:  # we had a response
            L2.addData('TR_Key.rt', TR_Key.rt)
        L2.addData('TR_Key.started', TR_Key.tStartRefresh)
        L2.addData('TR_Key.stopped', TR_Key.tStopRefresh)
        L2.addData('TR_Image.started', TR_Image.tStartRefresh)
        L2.addData('TR_Image.stopped', TR_Image.tStopRefresh)
        ...
        for thisComponent in TR_WComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        L2.addData('TR_Wait.started', TR_Wait.tStartRefresh)
        L2.addData('TR_Wait.stopped', TR_Wait.tStopRefresh)
        thisExp.nextEntry()
        
    # completed 1 repeats of 'L2'
    ...
    for thisComponent in Actual_InstructionsComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    L1.addData('AR_Instructions.started', AR_Instructions.tStartRefresh)
    L1.addData('AR_Instructions.stopped', AR_Instructions.tStopRefresh)
    # check responses
    if key_resp_2.keys in ['', [], None]:  # No response was made
        key_resp_2.keys = None
    L1.addData('key_resp_2.keys',key_resp_2.keys)
    if key_resp_2.keys != None:  # we had a response
        L1.addData('key_resp_2.rt', key_resp_2.rt)
    L1.addData('key_resp_2.started', key_resp_2.tStartRefresh)
    L1.addData('key_resp_2.stopped', key_resp_2.tStopRefresh)
    # the Routine "Actual_Instructions" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    
    # set up handler to look after randomisation of conditions etc
    L3 = data.TrialHandler(nReps=1, method='sequential', 
        extraInfo=expInfo, originPath=-1,
    ...
    for thisL3 in L3:
        ...
        for thisComponent in ARComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        L3.addData('AR_Image.started', AR_Image.tStartRefresh)
        L3.addData('AR_Image.stopped', AR_Image.tStopRefresh)
        # check responses
        if AR_Key.keys in ['', [], None]:  # No response was made
            AR_Key.keys = None
        L3.addData('AR_Key.keys',AR_Key.keys)
        if AR_Key.keys != None:  # we had a response
            L3.addData('AR_Key.rt', AR_Key.rt)
        L3.addData('AR_Key.started', AR_Key.tStartRefresh)
        L3.addData('AR_Key.stopped', AR_Key.tStopRefresh)
        ...
        for thisComponent in AR_WComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        L3.addData('AR_Wait.started', AR_Wait.tStartRefresh)
        L3.addData('AR_Wait.stopped', AR_Wait.tStopRefresh)
        thisExp.nextEntry()
        
    # completed 1 repeats of 'L3'
    
    thisExp.nextEntry()
    
# completed 2 repeats of 'L1'
